...
class ScreenSharingServer:
    def __init__(self, master, host="0.0.0.0", port=5001):
        ...
        self.flashBang = objTTK.Button(
            objSettingsTab5, text="FLASHBANG!", command=self.flashbang
        )
        ...
        self.messageTheClientButton = objTTK.Button(
            objSettingsTab5, text="Message!", command=self.messageTheClient
        )
            ...
            while True:
                ...
                if empty_slot is not None:
                    ...
                    self.update_client_labels(
                        empty_slot, address[0], ipv6, connectionsNum
                    )  # Update client label
                    threading.Thread(
                        target=self.receive_images, args=(client_socket, empty_slot)
                    ).start()
                else:
